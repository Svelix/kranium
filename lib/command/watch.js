...
function watch(opts){
	...
	// Export options for App
	Bootstrap.createAppBootstrap(opts);

	// Check for file changes every 100ms, convert and send to app
	var interval = setInterval(processChanges, 100);

	// Create server
	var appsocket = net.createServer(function(stream) {
...
var fileTypes = "js jss kss styl less sass coffee".split(" "),
...
// Read a file and pass an object with name and content to the callback.
...
// Reads either all or all modified files and passes an array of objects with the name and contect of each file.
//  * type = 'new': read only modified files
//  * type = 'all': read all files
...
// Convert styl/less/sass/coffee files to kss/js files.
// Files are put into the same directory as the original file.
// The callback is called for every processed file with an object of name and content.
function processFiles(err, files, callback){
	async.map(files, function(o, callback){
		...
		switch(ext){
			...
			case 'sass':
				var kssfile = filename.replace(/sass$/, 'kss');
				var cmd = 'sass '+filename+' '+kssfile;
				exec(cmd, function(error, stdout, stderr){
					if (error) {
						console.log(stderr.replace(/^|\n/g, "$&[ERROR] in file"+filename+': '));
						return callback(error);
					}
					css = fs.readFileSync(kssfile);
					return callback(null, { name: kssfile, content: css });
				});
				break;
...
// Read all changed files, convert them and send them to the app.
